...
impl Type {
    ...
    pub fn from_expr<Info>(ex: &ast::Expr<Info>) -> Option<Vec<Type>>
    where
        Info: std::fmt::Debug + Clone + PartialEq + Eq,
    {
        match &ex {
            ast::Expr::UnaryOp(ast::UnaryOp { expr, op, .. }) if op == &ast::PrefixOp::Deref => {
                if let Some(mut tys) = Type::from_expr(expr.expr()) {
