...
// impl<'a> IntoHirDecl for &'a Node<Decl> {
//     type Output = Vec<Node<HirDecl>;
...
//                 let ty = n.ty.as_ref().unwrap().clone_value().resolve_ty(scope, ctx);
...
//                             ty.clone_value().resolve_ty(&struct_path, &mut struct_ctx)
//                         } else {
//                             return Err(RayError {
...
//                         n,
//                         p.ty().map(|t| t.clone().resolve_ty(&func_fqn, &mut fn_ctx)),
//                     ));
//                 }
...
//                 let (name, ty_params) = match tr.ty.clone_value().resolve_ty(scope, ctx) {
...
//                     .super_trait
//                     .clone()
//                     .map(|t| t.take_value().resolve_ty(&parent_scope, ctx));
...
//                     match imp.ty.clone_value().resolve_ty(&parent_scope, ctx) {
