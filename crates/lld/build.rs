...
lazy_static! {
    ...
    static ref LLVM_CONFIG_BINARY_NAMES: Vec<String> = {
        vec![
            "llvm-config".into(),
            format!("llvm-config-{}", LLVM_VERSION.major),
            format!("llvm-config-{}.{}", LLVM_VERSION.major, LLVM_VERSION.minor),
        ]
    };
            ...
            for binary_name in LLVM_CONFIG_BINARY_NAMES.iter() {
                ...
                if is_compatible_llvm(&ver) {
                    ...
                    println!("LLVM binaries specified by {} are the wrong version.
                              (Found {}, need {}.)", binary_prefix_var, ver, *LLVM_VERSION);
                }
            }
        }
...
fn is_compatible_llvm(llvm_version: &Version) -> bool {
    ...
    if strict {
        llvm_version.major == LLVM_VERSION.major && llvm_version.minor == LLVM_VERSION.minor
    } else {
        llvm_version.major >= LLVM_VERSION.major
            || (llvm_version.major == LLVM_VERSION.major
                && llvm_version.minor >= LLVM_VERSION.minor)
    }
}
