    ...
    V: TyVar,
{
    ...
    pub var_kinds: HashMap<V, VarKind>,
}
    ...
    V: Display + Debug + TyVar,
{
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        if self.var_kinds.is_empty() {
            write!(f, "  var_kinds: [],\n")?;
        } else {
            write!(f, "  var_kinds: [\n")?;
            for (v, kind) in &self.var_kinds {
                write!(f, "    {}: {:?}\n", v, kind)?;
            }
            write!(f, "  ],\n")?;
        }
