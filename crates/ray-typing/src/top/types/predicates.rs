...
use crate::top::{TyVar, mgu_ref_slices};
    ...
    V: TyVar,
{
    ...
    pub fn by_instance(
        ...
        synonyms: &OrderedTypeSynonyms<T, V>,
    ) -> Option<Vec<(Subst<V, T>, Option<Vec<Predicate<T, V>>>)>>
    where
        V: Display,
        <V as FromStr>::Err: Debug,
    {
        log::debug!("by_instance: predicate = {}", predicate);
        let mut instance_candidates = vec![];

        match predicate {
            Predicate::Class(class_name, _, _, _) => {
                let Some(instances) = self.instances(class_name) else {
                    log::debug!("by_instance: no instances for predicate: {}", predicate);
                    return None;
                };
                for instance in instances.iter() {
                    log::debug!("by_instance: instance = {:?}", instance);
                    instance_candidates.push((&instance.head, Some(&instance.predicates)));
                }
            }
            ...
            Predicate::HasField(_, field_name, _, _) => {
                ...
                for pred in record_preds.into_iter() {
                    instance_candidates.push((pred, None));
                }
            }
            Predicate::Recv(..) => return None,
        }

        let mut candidates = vec![];
        for (candidate, additional_preds) in instance_candidates {
            let Some(subst) = predicate.match_with(candidate, synonyms) else {
                log::debug!(
                    "by_instance: predicates do not match: predicate={} & candidate={}",
                    predicate,
                    candidate
                );
                continue;
            ...
            let predicates = if let Some(additional_preds) = additional_preds {
                ...
                Some(predicates.into())
            } else {
                None
            };

            candidates.push((subst, predicates));
        }
    ...
    V: TyVar,
{
    ...
    where
        ...
        <V as FromStr>::Err: Debug,
    {
        log::debug!("[match_with] {} ~ {}", self, other);
        match (self, other) {
            (
