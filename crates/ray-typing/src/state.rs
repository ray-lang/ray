...
use crate::top::{self, Subst, Substitutable};
...
impl TyVarFactory {
    ...
    pub fn skip_to(&mut self, value: u32) {
        self.value = value;
    }

    pub fn next(&mut self) -> TyVar {
        self._next_with_prefix(None)
    }

    pub fn next_with_prefix(&mut self, prefix: &str) -> TyVar {
        self._next_with_prefix(Some(prefix))
    }

    fn _next_with_prefix(&mut self, prefix: Option<&str>) -> TyVar {
        let v = self.value;
        self.value += 1;
        let name = format!("{}{}", prefix.unwrap_or(self.prefix.as_str()), v);
