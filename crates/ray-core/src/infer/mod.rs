...
use ray_typing::top::{
    Class, Instance, Predicate, Predicates, Subst, Substitutable, mgu,
...
use crate::collect::{CollectConstraints, CollectCtx};
...
impl<'a> InferSystem<'a> {
        ...
        T: CollectConstraints<Output = U> + std::fmt::Display,
    {
        ...
        options.schema_vars = self.tcx.get_schema_vars();
        ...
        for (_, struct_ty) in self.tcx.structs() {
            log::debug!(
                "[infer_ty] converting structs to record class: struct_ty={:?}",
                struct_ty
            );
