...
use tower_lsp::{
    ...
    lsp_types::{
        ...
        Location, MarkupContent, MarkupKind, MessageType, Position, Range, SemanticTokens,
...
use ray::{libgen, pathlib::FilePath, span::Span};
use serde_json::Value;

use crate::{diagnostics::AnalysisSnapshotData, semantic_tokens::pretty_dump};
...
fn uri_to_filepath(uri: &Url) -> Option<FilePath> {
    if let Ok(path) = uri.to_file_path() {
...
fn span_to_range(mut span: Span) -> Range {
...
fn filepath_to_uri(filepath: &FilePath) -> Option<Url> {
