...
# Build the wasi_malloc crate as a relocatable WASM object for linking
...
echo "==> building wasi_malloc (target: $WASI_TARGET)"

# --- Build the relocatable .wasm object ---
TARGET_DIR="$PROJECT_DIR/target/wasi-malloc"
cd "$PROJECT_DIR/crates/wasi_malloc"
...
cp "$TARGET_DIR/$WASI_TARGET/release/wasi_malloc.wasm" \
    "$PROJECT_DIR/lib/libc/wasi_malloc.wasm"
